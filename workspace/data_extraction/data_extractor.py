...
class DataExtractorNode(Node):
    ...
    def callback(self, road_msg, nonroad_msg):
        ...
        # 2. Fusion pour l'INPUT (X, Y, Z, I)
        all_points = np.concatenate([road_pts, nonroad_pts], axis=0)

        # 3. Création du MASQUE TARGET (BEV)
        # On utilise les dimensions de ton modèle PointPillars
        pc_range = [0, -39.68, -3, 69.12, 39.68, 1]
        voxel_size = [0.16, 0.16]
        grid_size = [432, 496] # X: 69.12/0.16=432, Y: 79.36/0.16=496

        mask = np.zeros((grid_size[0], grid_size[1]), dtype=np.uint8)

        # Projection des points road uniquement
        res_x = (road_pts[:, 0] - pc_range[0]) / voxel_size[0]
        res_y = (road_pts[:, 1] - pc_range[1]) / voxel_size[1]
        ...
        # --- ASTUCE : Épaissir un peu pour boucher les trous du centre lors du training ---
        kernel = np.ones((3,3), np.uint8)
        mask = cv2.dilate(mask, kernel, iterations=1)

        # 4. Sauvegarde
        np.save(os.path.join(self.save_path, 'points', f"{self.frame_idx:04d}.npy"), all_points)
        cv2.imwrite(os.path.join(self.save_path, 'masks', f"{self.frame_idx:04d}.png"), mask)

        if self.frame_idx % 10 == 0:
            #self.get_logger().info(f"Frame {self.frame_idx} enregistrée...")
        self.frame_idx += 1
