...
class DriverFocusNode(Node): 
    ...
    def __init__(self):
        ...
        # === Subscription au flux vidéo ROS2 ===
        self.image_sub = self.create_subscription(
            Image,
            '/mako/image_raw',  # Topic existant de la caméra
            self.image_callback,
            10
        )
        self.frame = None
        ...
        # === Timer (~30 FPS) pour traiter les frames ===
        self.timer = self.create_timer(0.033, self.loop)

    def image_callback(self, msg: Image):
        # Convertir CompressedImage ROS -> OpenCV
    ...
    def loop(self):
        ...
        if landmarks:
            ...
            if not self.calibration.is_calibrated():
                ...
                yaw_rel, pitch_rel, forward_head = self.head_pose.estimate_relative_angles(
                    landmarks.landmark,
                ...
                score_to_publish = self.scorer.compute_score(
                    initial_score=head_score,
                    penalty=yawn_penalty + fatigue_penalty
                )
                
                #self.get_logger().info(str(score_to_publish))
