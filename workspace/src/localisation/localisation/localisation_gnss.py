...
#Fonction globale, celle que je veux importer
...
def plot_map_with_point(segments_enu, point_enu, closest_segment=None):
    ...
    px, py, _ = point_enu
    plt.scatter(px, py, s=120, edgecolors='black', facecolors='yellow', zorder=5, label="voiture")
    plt.xlabel("E (m)")
    plt.ylabel("N (m)")
    plt.title("Carte ENU + Position voiture")
    plt.legend()
    plt.axis("equal")
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    csv_enu = "SevilleCoubureSignee.csv"
    ...
    #Point enu exemple
    #point_enu=(np.float64(-150), np.float64(87.5), np.float64(1.497395704779791))
